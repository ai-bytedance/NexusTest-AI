...
class GLMProvider(AIProvider):
    ...
    def _invoke_structured(
        ...
        try:
            ...
            self.logger.error("glm_json_parse_error", content=content)
            raise AIProviderError("Unable to parse GLM response as JSON", data={"content": content}) from exc
        ...
        with httpx.Client(timeout=self.timeout) as client:
            while attempt < self.max_retries:
                ...
                try:
                    ...
                    self.logger.warning("glm_timeout", attempt=attempt)
                    if attempt >= self.max_retries:
                        raise AIProviderTimeoutError("GLM request timed out") from exc
                    self._sleep(attempt)
                    continue
                except httpx.HTTPError as exc:
                    self.logger.error("glm_transport_error", error=str(exc))
                    raise AIProviderUnavailableError("GLM transport error") from exc

                if response.status_code == 429:
                    self.logger.warning("glm_rate_limited", attempt=attempt)
                    if attempt >= self.max_retries:
                        raise AIProviderRateLimitError(
                            "GLM rate limit exceeded",
                ...
                if 500 <= response.status_code < 600:
                    self.logger.warning("glm_server_error", status_code=response.status_code, attempt=attempt)
                    if attempt >= self.max_retries:
                        raise AIProviderUnavailableError(
                            "GLM service unavailable",
                            data=self._safe_json(response),
                        )
                    self._sleep(attempt)
                    continue
                ...
                try:
                    ...
                    raise AIProviderError(message or "GLM request failed", data=error_payload) from exc

                return self._safe_json(response)

        raise AIProviderUnavailableError("GLM service unavailable")

    def _extract_content(self, response: dict[str, Any]) -> str:
        choices = response.get("choices")
        if not isinstance(choices, list) or not choices:
            raise AIProviderError("GLM response missing choices", data=response)
        first_choice = choices[0]
        if not isinstance(first_choice, dict):
            raise AIProviderError("GLM response missing message", data=response)
        message = first_choice.get("message")
        if not isinstance(message, dict):
            raise AIProviderError("GLM response missing message content", data=response)
        ...
        if not isinstance(content, str):
            raise AIProviderError("GLM response missing content", data=response)
