...
class ClaudeProvider(AIProvider):
    ...
    def _invoke_structured(
        ...
        try:
            ...
            self.logger.error("anthropic_json_parse_error", content=content)
            raise AIProviderError("Unable to parse Claude response as JSON", data={"content": content}) from exc
        ...
        with httpx.Client(timeout=self.timeout) as client:
            while attempt < self.max_retries:
                ...
                try:
                    ...
                    self.logger.warning("anthropic_timeout", attempt=attempt)
                    if attempt >= self.max_retries:
                        raise AIProviderTimeoutError("Claude request timed out") from exc
                    self._sleep(attempt)
                    continue
                except httpx.HTTPError as exc:
                    self.logger.error("anthropic_transport_error", error=str(exc))
                    raise AIProviderUnavailableError("Claude transport error") from exc

                if response.status_code == 429:
                    self.logger.warning("anthropic_rate_limited", attempt=attempt)
                    if attempt >= self.max_retries:
                ...
                if 500 <= response.status_code < 600:
                    self.logger.warning("anthropic_server_error", status_code=response.status_code, attempt=attempt)
                    if attempt >= self.max_retries:
                ...
                try:
                    ...
                    raise AIProviderError(message or "Claude request failed", data=error_payload) from exc

                return self._safe_json(response)

        raise AIProviderUnavailableError("Claude service unavailable")

    def _extract_content(self, response: dict[str, Any]) -> str:
        content = response.get("content")
        if not isinstance(content, list) or not content:
            raise AIProviderError("Claude response missing content", data=response)
        texts: list[str] = []
        for item in content:
        ...
        if not texts:
            raise AIProviderError("Claude response missing textual content", data=response)
        return "\n".join(texts).strip()
